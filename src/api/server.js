...
const warmingCron = require('../../cron/warming.cron');
 ...
 */
app.get('/api/warming/status', async (req, res) => {
    try {
        const poolStatus = await warmingPool.getStatus();
        const workerStatus = warmingWorker.getStatus();
        const proxies = await warmingPool.getProxies();
        ...
        res.json({
            ...
            proxies: {
                ...
                available: proxies.filter(p => !p.assigned_account_id).length
            }
        });
    } catch (error) {
        logger.error('Error in GET /api/warming/status:', error);
        res.status(500).json({ error: error.message });
    }
});

/**
 * Add account to warming pool
 * POST /api/warming/accounts
 * Body: { username, password, totpSecret? }
 */
app.post('/api/warming/accounts', async (req, res) => {
    try {
        ...
        const account = await warmingPool.addAccount(username, password, totpSecret);

        res.status(201).json({
            message: 'Account added to warming pool',
            account: {
                id: account.id,
                username: account.username,
                status: account.status,
                hasProxy: !!account.proxy_id
            }
        });
    } catch (error) {
        logger.error('Error in POST /api/warming/accounts:', error);
        res.status(500).json({ error: error.message });
    }
});

/**
 * Get warming accounts list
 * GET /api/warming/accounts
 */
app.get('/api/warming/accounts', async (req, res) => {
    try {
        const status = await warmingPool.getStatus();
        res.json(status);
    } catch (error) {
        logger.error('Error in GET /api/warming/accounts:', error);
        res.status(500).json({ error: error.message });
    }
});

/**
 * Manually trigger warming for an account
 * POST /api/warming/accounts/:accountId/warm
 */
app.post('/api/warming/accounts/:accountId/warm', async (req, res) => {
    try {
        const { accountId } = req.params;

        if (warmingWorker.isRunning) {
            return res.status(409).json({
                error: 'Warming worker is already running',
                currentAccount: warmingWorker.currentAccount
            });
        }

        // Check if within allowed hours
        if (!warmingCron.isWithinWarmingHours()) {
            return res.status(400).json({
                error: 'Outside warming hours (08:00-23:00 Brasília)',
                hint: 'Manual warming is only allowed during designated hours'
            });
        }

        const result = await warmingCron.triggerManualWarm(accountId);

        res.json({
            message: 'Warming session triggered',
            result
        });
    } catch (error) {
        logger.error('Error in POST /api/warming/accounts/:accountId/warm:', error);
        res.status(500).json({ error: error.message });
    }
});

/**
 * Promote account to production
 * POST /api/warming/accounts/:accountId/promote
 */
app.post('/api/warming/accounts/:accountId/promote', async (req, res) => {
    try {
        const { accountId } = req.params;

        const success = await warmingPool.promoteToProduction(accountId);

        if (success) {
            res.json({ message: 'Account promoted to production successfully' });
        } else {
            res.status(400).json({ error: 'Failed to promote account' });
        }
    } catch (error) {
        logger.error('Error in POST /api/warming/accounts/:accountId/promote:', error);
        res.status(500).json({ error: error.message });
    }
});
...
/**
 * Add warming proxy
 * POST /api/warming/proxies
 * Body: { host, port, username?, password? }
 */
app.post('/api/warming/proxies', async (req, res) => {
    try {
        ...
        const proxy = await warmingPool.addProxy(host, parseInt(port), username, password);

        res.status(201).json({
            message: 'Warming proxy added',
            proxy: {
                id: proxy.id,
                host: proxy.host,
                port: proxy.port
        ...
        logger.error('Error in POST /api/warming/proxies:', error);
        res.status(500).json({ error: error.message });
    }
});
...
/**
 * Get warming proxies
 * GET /api/warming/proxies
 */
app.get('/api/warming/proxies', async (req, res) => {
    try {
        const proxies = await warmingPool.getProxies();
        res.json({
            total: proxies.length,
            proxies: proxies.map(p => ({
                ...
                assignedAccount: p.assigned_account_id
            }))
        });
    } catch (error) {
        logger.error('Error in GET /api/warming/proxies:', error);
        res.status(500).json({ error: error.message });
    }
});

/**
 * Check and promote ready accounts (admin)
 * POST /api/warming/promote-ready
 */
app.post('/api/warming/promote-ready', async (req, res) => {
    try {
        const promoted = await warmingPool.checkAndPromoteReadyAccounts();
        res.json({
            message: `Promoted ${promoted.length} accounts`,
            promoted
        });
    } catch (error) {
        logger.error('Error in POST /api/warming/promote-ready:', error);
        res.status(500).json({ error: error.message });
    }
});
...
const server = app.listen(PORT, () => {
    ...
    setTimeout(async () => {
        try {
            ...
            warmingCron.scheduleWarmingJobs();
            logger.info('Warming CRON jobs scheduled');
        } catch (e) {
